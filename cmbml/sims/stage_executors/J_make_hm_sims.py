...
class SimHMCreatorExecutor(BaseStageExecutor):
    """
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        super().__init__(cfg, stage_str='make_hm_sims')
        ...
        for freq, detector in self.instrument.dets.items():
            with self.name_tracker.set_context("freq", freq):
                with self.name_tracker.set_context("hm", 1):
                    noise_maps_hm1 = self.in_noise.read(map_field_strs=detector.fields, use_alt_path=split.noise_fixed)
                with self.name_tracker.set_context("hm", 2):
                    noise_maps_hm2 = self.in_noise.read(map_field_strs=detector.fields, use_alt_path=split.noise_fixed)
            ...
            obs_maps_hm1 = noise_maps_hm1.to(self.output_units, equivalencies=u.cmb_equivalencies(detector.cen_freq))
            obs_maps_hm1 += sky_no_noise_maps.to(self.output_units, equivalencies=u.cmb_equivalencies(detector.cen_freq))
            obs_maps_hm2 = noise_maps_hm2.to(self.output_units, equivalencies=u.cmb_equivalencies(detector.cen_freq))
            obs_maps_hm2 += sky_no_noise_maps.to(self.output_units, equivalencies=u.cmb_equivalencies(detector.cen_freq))

            with self.name_tracker.set_contexts(dict(freq=freq, hm=1)):
                self.out_obs.write(data=obs_maps_hm1, column_names=column_names)
            with self.name_tracker.set_contexts(dict(freq=freq, hm=2)):
                self.out_obs.write(data=obs_maps_hm2, column_names=column_names)
