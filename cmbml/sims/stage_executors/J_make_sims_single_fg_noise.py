...
class SingleNoiseSimCreatorExecutor(BaseStageExecutor):
    """
    ...
    def load_fixed_split_maps(self):
        for freq, detector in self.instrument.dets.items():
            with self.name_tracker.set_context("freq", freq):
                noise_maps = self.in_noise.read(map_field_strs=detector.fields, use_alt_path=True)
                self.noise_maps[freq] = self.apply_filter(noise_maps, freq)
    ...
    def process_sim(self, split: Split, sim_num: int) -> None:
        ...
        for freq, detector in self.instrument.dets.items():
            with self.name_tracker.set_context("freq", freq):
                noise_maps = self.noise_maps[freq]
