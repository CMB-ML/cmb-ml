...
class ObsCreatorPlFGsExecutor(BaseStageExecutor):
    """
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        super().__init__(cfg, stage_str='make_obs_no_noise_pl_fgs')
        ...
        # Do not load foreground maps here, do this in the execute() method
    ...
    def execute(self) -> None:
        ...
        for freq, det in self.instrument.dets.items():
            with self.name_tracker.set_contexts(dict(freq=freq)):
                fg_map = self.in_fg_maps.read()[0]
                fg_map = fg_map.to(u.uK_CMB,
                                   equivalencies=u.cmb_equivalencies(det.cen_freq))
                self.foregrounds[freq] = fg_map
    ...
    def process_sim(self, split: Split, sim_num: int) -> None:
        ...
        cmb_realization = cmb.map[0]  # Get temperature map only
        ...
        for freq, detector in self.instrument.dets.items():
            ...
            fg = self.foregrounds[freq]
            sky_map = fg + cmb_realization

            # Use pysm3.apply_smoothing... to convolve the map with the planck detector beam
            map_smoothed = pysm3.apply_smoothing_and_coord_transform(sky_map,
                                                                     detector.fwhm,
                                                                     output_nside=self.nside_out)
            final_map = map_smoothed
    ...
    def save_cmb_map_realization(self, cmb: CMBLensed, min_fwhm):
        ...
        # PySM3 components always include T, Q, U, so we extract the temperature map
